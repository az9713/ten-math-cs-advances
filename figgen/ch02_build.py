"""Assemble ch02-binary-spherical-codes.html from part files + SVGs.

Parts: .ignore/ch02_parts/p*.html (sorted). Markers <!--FIG:name--> are
replaced by .ignore/ch02_figs/name.svg content; the marker <!--LISTING-->
is replaced by the HTML-escaped contents of figgen/ch02_checks.py wrapped
in the codewrap/copy-button block. Idempotent; rerun any time.
Usage: PYTHONIOENCODING=utf-8 python figgen/ch02_build.py
"""
import glob
import io
import os
...
PARTS = os.path.join(ROOT, ".ignore", "ch02_parts")
FIGS = os.path.join(ROOT, ".ignore", "ch02_figs")
LISTING = os.path.join(ROOT, "figgen", "ch02_checks.py")
OUT = os.path.join(ROOT, "ch02-binary-spherical-codes.html")
